...
## 1.11.2
+ 添加了对22.5.0的基本支持。
+ Atmosphère 已更新为使用 GCC 16/newlib（最新的 devkitA64/devkitARM 版本）。
+ 系统稳定性的整体改进，以提升用户体验。
## 1.11.1
+ 添加了对22.1.0的基本支持。
+ 系统稳定性的整体改进，以提升用户体验。
## 1.11.0
+ 添加了对22.0.0的基本支持。
+ 特别感谢@alula处理了大量必要的内核、加载器和erpt更改。
  + 主机应能启动且Atmosphere应完全功能正常。
+ **请注意**：应用程序/小程序生命周期管理方式的改变破坏了自制软件生态系统。
    + 所有应用程序和小程序都应通过调用相关的进程间通信（IPC）命令进行正常退出。
    + 使用libnx编译的自制软件以及hbmenu本身会明确避免退出，以便能持续使用相同的进程。
      + 要妥善解决这个问题，需要重新设计自制软件的启动和终止方式。
    + 作为临时解决方案，现已包含对`am`系统模块的补丁，该补丁可恢复先前的行为，且无需任何进一步更改即可恢复自制软件的兼容性。
    + 尽管如此，在测试自制软件时若遇到任何问题，请予以报告，以确保没有遗漏任何极端情况。  
  + `exosphère`已更新，以反映最新的官方安全监控器行为。
  + `mesosphère`已更新，以反映最新的官方内核行为。
  + `loader`已更新，以反映最新的官方行为。
  + `erpt`已更新，以反映最新的官方行为。
  + `pgl`已更新，以修复一些问题。
+ 增加了对4GB以上内存模式的支持（感谢@CTCaer）。
+ 增加了一个构建时选项，可将Joy-Con滑轨配置为用于调试的通用异步收发传输器（UART）（感谢@alula）。
+ 对系统稳定性进行了全面改进，以提升用户体验。 
## 1.10.2
+ 添加了对21.2.0的基本支持。
+ 系统稳定性的整体改进，以提升用户体验。
## 1.10.1
+ 添加了对21.1.0的基本支持。
+ 修复了一个导致部分游戏（如《古墓丽影》终极版）无法启动的漏洞。
+ 系统稳定性的整体改进，以提升用户体验。
## 1.10.0
+ 添加了对21.0.0的基本支持。
  + 主机应能启动且Atmosphere应完全功能正常。
  + **请注意**：所有自制软件可能需要使用最新版libnx（>=4.10.0）重新编译，否则可能崩溃/遇到内存损坏问题。
    + 任天堂在21.0.0中破坏了用户态<->内核TLS ABI，占用了先前保留的空间。
    + 自制软件曾使用此保留空间作为TLS插槽，这意味着任何使用TLS插槽的自制软件在Atmosphere 1.10.0下运行时将遇到内存损坏。
    + 这似乎不会影响所有软件，但大部分测试的自制软件会崩溃（通常在退出时），因此需要为新ABI重新编译。
      + 对于技术爱好者，虽然开发者很少使用TLS插槽，但它们用于实现如C++异常等底层功能，因此任何使用这些功能的软件都会崩溃等。
    + 为简化过渡，hbmenu现在在选择使用旧版不兼容ABI编译的自制软件时会显示警告。
      + 对此造成的不便，我深表歉意。
      + libnx已更新，其保留空间现在与任天堂匹配——即使任天堂再占用更多保留空间，此特定问题也永远不会再发生。
  + `exosphère`更新以反映最新的官方安全监视器行为。
  + `mesosphère`更新以反映最新的官方内核行为。
  + `loader`更新以反映最新的官方行为。
  + `pm`更新以反映最新的官方行为。
  + `erpt`更新以反映最新的官方行为。
  + `pgl`更新以反映最新的官方行为。
  + `fatal`更新以反映最新的官方行为。
+ 添加了对启动另一个文件过多的游戏（带romfs修改）的支持。
  + 我依赖用户报告来添加支持/修复这些问题，其中一些游戏可能相当冷门！
  + 如果您受此影响，在尝试使用mod启动游戏时会看到"Data abort (0x101)"错误。
  + 如果发生此情况，请通过Discord联系`sciresm`分享错误报告二进制文件。
    + 尽管某些游戏可能无法修复，但我相信几乎可以解决所有问题，所以如果您遇到此问题，请让我尝试帮助您（并改进Atmosphere的支持！）。
+ 常规系统稳定性改进，以提升用户体验。
